...
public class EmiTags {
    ...
    private static @Nullable String translatePrefix(String prefix, ResourceLocation id) {
        ...
        if (id.getResourcePath().equals("forge")) {
...
//            if (parts.length > 1) {
//                TagKey<?> key = TagKey.of(RegistryKey.ofRegistry(EmiPort.id("minecraft", parts[0])), EmiPort.id(id.getResourceDomain(), path.substring(1 + parts[0].length())));
